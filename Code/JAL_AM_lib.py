...
def run_sim_JAL_AM(n, k, show_progress=False):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        if show_progress:
            iterator = tqdm(enumerate(as_completed(futures)), total=n, desc='JAL-AM')
        else:
...
def run_sim_JAL_AM_asym(n, k, mu,show_progress=False):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        if show_progress:
            iterator = tqdm(enumerate(as_completed(futures)), total=n, desc='JAL-AM asym')
        else:
            iterator = enumerate(as_completed(futures))
...
@njit
def JAL_AM_asym_convergence(alpha, gamma, T, price_grid, mu):
    """
    args
        alpha: step-size parameter
        gamma: discount factor
